...
> **声明**：本项目仅供学习交流。使用前请自行确认目标站点服务条款；账号与密码请只放在 GitHub Secrets / 本地环境变量中，切勿写入代码或提交到仓库。
...
1. **永远不要**把账号、密码、JWT、Cookie 写进代码或 README
2. 只用 GitHub Secrets / 本地环境变量
3. 若曾在聊天、截图、旧提交里暴露过密码，**先改密再开源**
4. 公开仓库前检查是否误提交 `.env`、抓包文件、浏览器导出
5. 不要在 workflow 里 `echo` 密钥
6. 凭据只放 Secrets；公开仓库更要严格
